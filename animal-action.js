...
// rock 장애물 감지 (주변 3개 바위 찾기) - 확장된 범위
function checkRockAheadForSnake() {
    ...
    const playerX = currentPlayer.position.x;
    const playerZ = currentPlayer.position.z;
    const detectionDistance = 25; // 앞쪽 25 단위까지 rock 장애물 감지 (기존 12에서 확장)
    const sideDetectionWidth = 12; // 좌우 12 단위까지 감지 (기존 5에서 확장)
    
    let nearbyRocks = [];

    // 플레이어 앞쪽의 rock 장애물들 체크 (확장된 범위)
    for (let obstacle of obstacles) {
        const obstacleX = obstacle.position.x;
        const obstacleZ = obstacle.position.z;
    ...
    // 가까운 순서로 정렬하고 최대 5개까지 선택 (기존 3개에서 확장)
    if (nearbyRocks.length > 0) {
        ...
        // 최대 5개까지 반환 (더 많은 바위 동시 공격 가능)
        return nearbyRocks.slice(0, 5);
    }
            ...
            if (hitDetected) {
                ...
                targets.forEach(rock => {
                    if (rock && rock.parent && !rock.userData.corroding) {
                        ...
                        scene.remove(rock);
                        console.log("돌이 독에 맞아 즉시 사라졌습니다!");
                    }
                });
